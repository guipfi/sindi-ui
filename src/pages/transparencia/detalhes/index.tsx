...
const Detalhes: NextPage = () => {
  ...
  return (
    <section className={styles.container}>
      ...
      {isErrorTransparenceDetails && (
        <Typography>Erro ao carregar as informações</Typography>
      )}
      {isLoadingTransparenceDetails ? (
        <Loader />
      ) : (
        <>
          <div className={styles.balanceInfos}>
            ...
            <div className={styles.monthIncomeAndOutcome}>
              <div className={styles.monthInfo}>
                <Typography variant={100}>Arrecadado no mês</Typography>
                <Typography variant={400}>
                  +{formatMoney(monthly_income ?? 0)}
                </Typography>
              </div>
              <div className={styles.monthInfo}>
                <Typography variant={100}>Gasto no mês</Typography>
                <Typography variant={400}>
          ...
          <div className={styles.selector}>
            <Typography className={styles.selected} variant={400} color="white">
              Gastos
            </Typography>
          </div>
          <div className={styles.incomeDetails}>
            <div className={styles.chart}>
              <BalanceDistribuitionChart
                data={relative_spendings_per_type ?? []}
              />
            </div>
            <DetailsList
              details={spendings}
              hasMarker={true}
              types={
                relative_spendings_per_type?.map((item) => item.type) ?? []
              }
            />
          </div>
        </>
