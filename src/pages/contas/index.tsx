...
const Contas: NextPage = () => {
  ...
  const { bill_amount, status, due_date, details, bar_code, pdf_url } =
    bill || {};
  ...
  return (
    <section className={styles.container}>
      ...
      {isBillLoading ? (
        ...
        <Typography>Sem fatura para exibir</Typography>
      ) : (
        <>
            ...
            <div className={styles.billStatus}>
              <i
                className={`${styles.marker} ${
                  styles[
                    status === 'open' || status === 'paid' ? 'green' : 'red'
                  ]
                }`}
              />
              <Typography variant={100}>{getStatusText()}</Typography>
            </div>
            <Typography variant={800}>
              {formatMoney(bill_amount ?? 0)}
            </Typography>
            <Typography variant={400}>
              Vence em {due_date?.getDate()} de{' '}
              {months[due_date?.getMonth() ?? 0]}
            </Typography>
          </div>
